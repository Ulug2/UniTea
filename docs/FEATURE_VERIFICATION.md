...
**Verified:** Manually, plus a full trace of the switching architecture (query key construction, per-community component mounting/keying, CSS layering of inactive panes) prompted by a user report of Community-feed posts briefly appearing in the Campus Feed until a manual refresh. `useFeedPosts`'s own community-scoping behavior (`community_id IS NULL` vs a specific id) is covered by Feature 4's tests. Confirmed the query keys, component instances (`key={feedKey}` per community, kept mounted and hidden via `opacity:0` for inactive panes — see the white-flash fix below for why this replaced an earlier `display:"none"` approach), and post-creation targeting (`resolvedCommunityId` threaded consistently from `create-post.tsx`) are all correctly isolated.
...
**Bug found & fixed (reported 2026-07-04, resolved 2026-08-01):** User reported that already-loaded images always flash a white/light placeholder before reappearing when switching Campus↔community or community↔community — but never when switching Hot/New/Top filters within the same feed, and never on text-only posts.

*First hypothesis, disproven:* the feed's `FlatList` had `removeClippedSubviews={true}`; since each pane stayed mounted behind a `display:"none"` View when inactive, a zero-size container could cause `removeClippedSubviews` to detach every cell's native views on top of that. Removed the prop — **user confirmed this did not resolve the issue.** (Kept off regardless: harmless, since `windowSize`/`initialNumToRender`/`maxToRenderPerBatch` already bound rendered cells.)

*Actual root cause:* a fresh investigation traced why filter switching (Hot/New/Top) never flashed while community switching always did, despite both claiming to keep inactive panes mounted. The mechanisms were not actually equivalent: filter panes stay laid out side-by-side inside a horizontal `ScrollView` and are only ever scrolled off-screen, never removed from layout. Community/Campus panes, by contrast, were hidden via `display:"none"` on the wrapping `View` (`feedLayerHidden` in `index.tsx`) — which collapses the node out of Yoga layout entirely. That layout removal was found to make the native image views beneath it lose their decoded bitmaps, so returning to an already-visited community forced `expo-image` to redecode from disk and replay its fade-in transition — visible as the reported white flash. Text-only posts have no image/placeholder/transition pipeline, so they were never affected.

*Fix applied:* replaced `feedLayerHidden`'s `display: "none"` with `opacity: 0` in `index.tsx`. This keeps inactive panes permanently laid out and mounted at the native level — matching what the working filter pager already did — so native image views are never torn down and never need to redecode on revisit. Touch-blocking on inactive panes was already handled by a separate `pointerEvents="none"` prop, unaffected by this change. Added `importantForAccessibility`/`accessibilityElementsHidden` on inactive panes to compensate for the one real behavioral difference `display:"none"` used to also provide for free: excluding hidden panes from the accessibility tree.
**Tests added:** None — this is native image-view/layout behavior not exercisable by the existing Jest/jest-expo setup; verified via source-level investigation (see the mechanism trace above) rather than an automated test.
