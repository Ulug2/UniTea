...
`ROADMAP.md` is the living product/bug plan. Whenever a commit relates to a roadmap item, update `ROADMAP.md` in that same commit: mark progress, add the commit hash, and move finished items to **Done** (and to **Waiting on a store build** if the change is client-side). Add newly reported bugs/features there when the user asks for them to be tracked.
