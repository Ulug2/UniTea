-- Store the primary image's aspect ratio (width / height) captured at upload time
-- so the feed can reserve the exact layout box on first render (zero CLS).
...
-- Recreate posts_summary_view to expose the new column.
